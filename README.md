...
## 최종 코드 설명
`final_quantize.py`는 EXAONE 4.0 모델에 GPTQ 양자화를 적용하는 코드입니다. 모델을 로드하고, 양자화 설정을 적용한 후, 양자화된 모델을 저장합니다. 주요 특징은 다음과 같습니다:
- 양자화 방식: W4A16
- 보호 레이어: 28번째 레이어의 up_proj
- Calibration 샘플 수: 256

## 실행 흐름
1. 필요한 라이브러리 설치 (transformers, auto-gptq 등)
2. 모델 로드
3. 양자화 설정 적용
4. GPTQ 양자화 수행
5. 양자화된 모델 저장
...
실행 예시:
```bash
python final_quantize.py
```

## 폴더 구조
- `README.md`: 프로젝트 설명
- `final_quantize.py`: 최종 양자화 코드
- `docs/`: 문서 폴더
...
## 실험 기록 문서 링크 안내
자세한 실험 시행착오 및 실패 원인은 `docs/experiment_log.md`를 참고하세요.
...
자세한 논문 리뷰는 `docs/paper_review.md`를 참고하세요.